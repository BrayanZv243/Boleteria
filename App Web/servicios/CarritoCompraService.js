export class CarritoCompraService {
    ...
    #urlCarritoCompra = 'http://localhost:3000/api/carrito-compras';

    async getCarritoCompraPorIDUsuario(token, idUsuario) {
        try {
            let res = await fetch(`${this.#urlCarritoCompra}/usuario/${idUsuario}`, {
                method: 'GET',
                headers: {
    ...
    async getBoletosDeUnCarritoCompra(token, idCarritoCompra) {
        try {
            let res = await fetch(`${this.#urlCarritoCompra}/${idCarritoCompra}/boletos`, {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
    ...
    async agregarBoletosACarritoCompra(token, idCarritoCompra, boletos) {
        ...
        try {
            ...
            const res = await fetch(`${this.#urlCarritoCompra}/${idCarritoCompra}/boletos`, requestOptions);
            if (res.ok) {
